...
import { IoCalendarOutline, IoPersonAddSharp, IoMegaphoneOutline, IoCaretForwardOutline } from "react-icons/io5"
...
const HomePage = () => {
  ...
  const handleNewArrivalDate = (new_arrivalDate) =>{
    if(departureDate < new_arrivalDate){
      setDepartureDate(new_arrivalDate);
    }
    setArrivalDate(new_arrivalDate);
  }

  const handleNewDepartureDate = (new_departureDate) =>{
    if(arrivalDate > new_departureDate){
      setArrivalDate(new_departureDate);
    }
  ...
  return (
    <Layout>
      ...
      <form className='bg-white shadow-md focus-within:shadow-lg border-r-2 p-4 m-2 flex duration-100 flex-wrap mt-10 md:pt-2'>
          ...
          <div className='p-2'>
            ...
            <div className="relative inline-block w-full my-2">
              ...
              <div className="pointer-events-none absolute inset-y-0 left-2 flex items-center px-2 text-gray-700">
                <FaBuilding className='h-5 w-5 text-third'></FaBuilding>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="group lg:w-1/3 w-full">
          <div className="p-2">
            <button className="bg-primary w-full block tracking-widest rounded text-white b font-medium md:mt-8 py-2 text-2xl uppercase duration-300 relative overflow-hidden group-hover:bg-primary-hover group-hover:-indent-12">
              <span >{t('search')}</span>
      ...
      <div className='block pt-4'>
        <div className='text-secondary text-xl my-2 py-3 font-bold'>
          {t('available_hotels')}
        </div>
        {
          HotelTowersListing.map((item, index) => (
            <HotelTowers key={index} image={item.image} title={item.title} desc={item.desc} />
          ))
        }
     </div>
      <PopupCalendar isOpen={isOpenCalendar} onClose={toggleCalendar} arrivalDate={arrivalDate} departureDate={departureDate} setArrivalDate={handleNewArrivalDate} setDepartureDate={handleNewDepartureDate} className={""} CalendarList={CalendarList}/>
    </Layout>
  );
};
